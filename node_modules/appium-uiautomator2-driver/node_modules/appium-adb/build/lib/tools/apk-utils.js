...
apkUtilsMethods.startUri = async function startUri(uri, pkg, opts = {}) {
    ...
    args.push('-a', 'android.intent.action.VIEW', '-d', (0, helpers_js_1.escapeShellArg)(uri), pkg);
    try {
        ...
        try {
            if (apiLevel < 23) {
                ...
                if (localeCode === curLocale) {
                    logger_js_1.default.debug(`Requested locale is equal to current locale: '${curLocale}'`);
                    return true;
                }
            }
            return false;
            ...
            logger_js_1.default.error(`Unable to check device localization: ${err.message}`);
            try {
...
apkUtilsMethods.getApkInfo = async function getApkInfo(appPath) {
    ...
    try {
        ...
        const { pkg, versionName, versionCode } = (0, helpers_js_1.parseManifest)(manifest);
        return {
            name: pkg,
            versionCode,
            versionName,
